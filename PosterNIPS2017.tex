...
\headerbox{Introduction}{name=intro,column=0}{
	...
	\begin{center}
		...
		$\epsilon = 0.02$
		\end{tabular}
	\captionof{figure}{Regularized optimal transport maps for decreasing regularization parameter $\epsilon$.}
	\end{center}
...
\headerbox{Sinkhorn-Knopp Algorithm}{name=Sinkhorn,column=0,below=intro}{
	...
	\end{gather*}
	As it is strictly convex, it has exactly one solution. It can be written equivalently as the \emph{Bregman projection} of a point onto a convex set:
	\begin{gather*}
	P_{\Ccal}(\xi) := \argmin_{\gamma \in \Ccal} \KL(\gamma,\xi),\\
	\gamma^* = P_{\Ccal_1 \cap \Ccal_2}(e^{-c/\epsilon}).
	\end{gather*}
	...
	Therefore the solution can be computed by alternating Bregman projections, as shown in [Bregman, 1964]:
	\[
