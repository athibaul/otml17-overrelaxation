...
\headerbox{Introduction}{name=intro,column=0,span=2}{
	\begin{minipage}{0.55\textwidth}
		...
		The algorithm we obtain converges between 2 and 20 times faster than Sinkhorn--Knopp, while remaining parallelizable, and having almost the same cost per iteration. Moreover, the log-domain techniques used to stabilize the Sinkhorn--Knopp algorithm with regard to numerical approximations {[Schmitzer, 2016]} can be directly extended to this algorithm.
	\end{minipage}
...
\headerbox{Lyapunov Function}{name=lyapunov,column=2}{
	Although the overrelaxation of iterative Bregman projections as presented here is already in use in the context of numerical calculations of optimal transport, there is no proof that this algorithm converges to the solution. In order to ensure this result, we use a Lyapunov function, and slightly modify the algorithm.
...
\headerbox{Experiments}{name=expe,below=lyapunov,column=2}{
	\begin{minipage}{\textwidth}
		We compare our algorithm to SK algorithm on two different settings.
		\begin{itemize}
			\item In setting (a) we consider the domain $[0,1]$ discretized into $100$ samples and the squared Euclidean cost.
			The marginals are randomly generated, piecewise constant on $[0,1]$.
	...
	\begin{minipage}{\textwidth}
		\begin{tcolorbox}[size=small]
			\begin{minipage}[b]{.5\linewidth}
				\centering
				\includegraphics[scale=0.35]{images/speedratio_image}\\
				{\centering\small (a) Quadratic cost, random marginals}\label{fig:compare_a}
			\end{minipage}%
			\begin{minipage}[b]{.5\linewidth}
				\centering
				\includegraphics[scale=0.35]{images/speedratio_ML}\\
...
\headerbox{References}{name=references,below=overrelaxation,column=1,bottomaligned=expe}{
	\small
	\begin{tabular}{r p{0.67\textwidth}}
		...
		{[Schmitzer, 2016]} & Schmitzer, B. 2016. Stabilized sparse scaling algorithms for entropy regularized transport problems. {\it arXiv preprint arXiv:1610.06519.}
	\end{tabular}
}
