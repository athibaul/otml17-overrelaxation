...
\begin{center}
\begin{tabular}{ccc}
\includegraphics[height=4cm]{schema_a.png}&\includegraphics[height=4cm]{schema_b.png}&\includegraphics[height=4cm]{schema_c.png}\\
(a)&(b)&(c)
\end{tabular}
...
\end{remark}
{\color{blue} As the inequality is strict in U, $\gamma^*\notin U$.} 
...
Using Schur complement properties, one can then show that any matrix $T(a,b)=(\diag(a)\diag(\gamma^0 b))^{-1}\gamma^0(diag(^t\gamma^*))^{-1} {^t\gamma^0}$ is positive semi definite. Moreover, we can show that $T$ has a unique largest eigenvalue that is $1$ and is associated to the eigenvector $\mathbf{1}$. All the other eigenvalues belongs to the set $[0;1[$.
...
In practice, we observed that the algorithm introduced in Theorem \ref{thm:algo} can be simplified and it is sufficient to check $F(P^\omega_{\Ccal_2}(P^\omega{\Ccal_1}(\gamma^l)))-F(\gamma^l)>0$ for realizing the over-relaxed iterations with parameter $\omega$.
...
\STATE $\bar a=a^{1-\omega}\otimes \tilde a^\omega$
\STATE $\tilde b=\mu_2\oslash (^t\gamma^0 \bar a)$
\IF{ $\langle \mu^1,\phi_\omega(a\oslash\tilde a) \rangle +\langle \mu^2,\phi_\omega(b\oslash\tilde b)\rangle>0$}
\STATE  $a=\bar a$ and $b=b^{1-\omega}\otimes \tilde b^\omega$
\COMMENT{Over-relaxed Sinkhorn iteration}
\ELSE
\STATE $a=\mu_1\oslash (\gamma^0 b)$ and $b=\mu_2\oslash (^t\gamma^0 a)$  
\COMMENT{Sinkhorn iteration}
\ENDIF
\ENDWHILE
...
Perspectives:
global convergence, how chosing $\omega$?, intensive numerical study with respect to other accelerarions methods\cite{}.
