...
%(our algorithm can also be parallelized on GPUs just as SK algorithm).

%In practice, we observed that the algorithm introduced in Theorem \ref{thm:algo} can be simplified and it is sufficient to check $F(P^\omega_{\Ccal_2}(P^\omega_{\Ccal_1}(\gamma^{\ell})))-F(\gamma^{\ell})>0$ for performing the overrelaxed iterations with parameter $\omega$.
