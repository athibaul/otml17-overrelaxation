...
{\color{blue}
From experiments, it seems possible to design a good guess for $\omega$ from $c$ and $\epsilon$ only. For $K=e^{-c/\epsilon}$, let us  define:
    $T=(diag(K\mathbf{1}))^{-1}K(diag(K^T\mathbf{1}))^{-1}K^T$. The matrix $T$ is positive semidefinite and checks
$\rho(T)=1$. Moreover  $T$ has  only one  eigenvalue $1$ associated to the eigenvector $\mathbf{1}$.
    Denoting as $\lambda$ the second larger eigenvalue of $T$, one can set 
