...
\item[$\bullet$] it is also an easy approach to having an adaptive method, as the approximation of $\Theta^*$ has a negligible cost (it only require to solve a one dimensional problem that depends on the smallest value of $(A_k \gamma)\oslash \mu^k$, which can be done in a few iterations of Newton's method).
...
In this context, when alternate maximization converges linearly at the rate $1-\eta$,  the local linear convergence rate of the SOR method is known to be $(1-\sqrt{\eta})/(1+\sqrt{\eta})$ ~\cite[Chapter 3]{chizat2017these}. 
